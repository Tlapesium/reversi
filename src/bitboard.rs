...
impl BitBoard {
    ...
    pub fn evaluate(&self, player: i32, player_now: i32) -> i32 {
        // 前半は置ける場所が多いほど良い
        // 後半は自分の石の個数を考慮する

        let moves = self.make_legal_board(player).count_ones() as i32;
        let mystones = self.board[player as usize].count_ones() as i32;
